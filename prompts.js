...
const structures = {
  ...
  twoSentence: (parts) => {
    if (parts.length === 2) {
      ...
      // For Turkish, if the first part ends with a colon, remove it and combine.
      const cleanedFirst = first.endsWith(':') ? first.slice(0, -1) : first;
      return punctuate(`${cleanedFirst} ${second}`);
    } else if (parts.length >= 4) {
      const first = punctuate(join(parts.slice(0, 3)));
      const second = punctuate(parts[3].trim());
      return `${first} ${second}`;
    }
    return punctuate(join(parts)); // Fallback for other cases
  },
